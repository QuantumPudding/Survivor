...
namespace TileEngine
{
    public class AStarPathfinding
    {
        ...
        {
            Map.Size = new Rectangle(0, 0, TileMap.CollisionLayer.Width, TileMap.CollisionLayer.Height);
            Map.Weight = new int[TileMap.CollisionLayer.Width, TileMap.CollisionLayer.Height];
            for (var y = 0; y < TileMap.CollisionLayer.Height; y++)
            {
                for (var x = 0; x < TileMap.CollisionLayer.Width; x++)
                {
                    Map.Weight[x, y] = TileMap.CollisionLayer.GetCellIndex(new Point(x, y));
                }
            }
        }
